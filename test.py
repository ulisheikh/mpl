...
def handle_all(message):
    ...
    if first_line.isdigit() and len(lines) > 1:
        ...
        for line in lines[1:]:
            ...
            # Ko'p so'zli so'zlarni qo'llab-quvvatlash
            parts = line.split(None, 1)  # Faqat birinchi bo'sh joyda bo'lish
            ...
            # Ikkala tomon ham bir xil tilmi? (ignore)
            if is_korean(w1) == is_korean(w2):
                continue
            
            # Korean va O'zbek
            kr, uz = (w1, w2) if is_korean(w1) else (w2, w1)
            data[topic_key][section_key][q_key][kr] = uz
            added += 1
    ...
    if uid in user_context and user_context[uid].get("topic") and user_context[uid].get("section") and user_context[uid].get("question"):
        ...
        # Bir qatorli so'z qo'shish
        parts = text.split(None, 1)  # Faqat birinchi bo'sh joyda bo'lish
        ...
        if is_korean(w1) == is_korean(w2):
            return bot.send_message(
                message.chat.id,
                "❌ Bir tomon korean, ikkinchisi o'zbek bo'lishi kerak"
            )
